function AllUpdated = updateSchedule(All, humanTime_filling, dist, vel_min, vel_max, inv_vel_min, inv_vel_max, idx_depot_tasks, idx_going_tasks, idx_to_ignore_r, idx_to_ignore_h, agents_ordered_allocation, service_time, humanTime_fillingPrev)
    ...
    for h=1:num_agents
        if humanData{h}.Task < num_filling_boxes
            ...
            humanTime_filling1(h:num_humans:end) = repmat(humanData{h}.FinishFilling(1) - humanData{h}.StartFilling(1), 1, num_filling_boxes);
            humanTime_serving(h:num_humans:end) = repmat(humanData{h}.FinishServing(1) - humanData{h}.StartServing(1), 1, num_filling_boxes);
    end 
    ...
    for curr_hum=1:num_tasks
        if All.timeFh(curr_hum) == FinishFill
            ...
            if humanTime_filling(curr_hum) > humanTime_fillingPrev(curr_hum)
                ...
                for s=curr_hum:num_agents:num_tasks
    ...
    for i=1:num_tasks
        ...
        elseif ismember(curr_hum, idx_to_ignore_h) %&& curr_hum_slow
            ...
            elseif curr_hum == human_to_shift - 1 && ~alreadyShifted
                ...
                All.timeS(wait) = All1.timeS(wait) + waiting_time(human_to_shift);
                All.timeF(wait) = All1.timeF(wait) + waiting_time(human_to_shift);

                All.timeS(app) = All1.timeS(app) + waiting_time(human_to_shift);
                All.timeF(app) = All1.timeF(app) + waiting_time(human_to_shift);

                All.timeS(serv) = All1.timeS(serv) + waiting_time(human_to_shift); %serving
                All.timeF(serv) = All1.timeF(serv) + waiting_time(human_to_shift);

                All.timeS(dep) = All1.timeS(dep) + waiting_time(human_to_shift);
                All.timeF(dep) = All1.timeF(dep) + waiting_time(human_to_shift);
            end
        end
