...
for h=1:num_humans
    ...
    humanData{h}.WaitingTime = repmat(randsample([-0.9, -0.5, 0.5, 0.9], 1), 1, num_filling_boxes); %Initialized with 1, it's the weight mapping to the Objective Function directly
...
function simulation(ReAll, idx_going_tasks, dist, vel_min, vel_max, inv_vel_min, inv_vel_max, idx_depot_tasks, service_time, num_tasks, idx_to_consider_r, idx_to_consider_h, idx_to_ignore_r, idx_to_ignore_h)
    ...
    num_phases = 5;
    duration = max(ReAllSave.timeSh(6:9)); %ReAll.makespan;
    valuetime = 0;
    waitingTime = [];
