...
num_filling_boxes = 3;
num_robots = 2;
initialTime = randi([120, 200], 1, num_agents);
...
function simulation(ReAll, idx_going_tasks, dist, vel_min, vel_max, inv_vel_min, inv_vel_max, idx_depot_tasks, service_time, num_tasks, idx_to_consider_r, idx_to_consider_h, idx_to_ignore_r, idx_to_ignore_h)
    ...
    num_phases = 5;
    duration = max(ReAllSave.timeSh(8:12)); %ReAll.makespan;
    valuetime = 0;
    waitingTime = [];
    ...
    while (valuetime <= duration)
        ...
        Experiments4Hum2Rob;
        duration = max(ReAllSave.timeSh(6:9)); %duration = ReAll.makespan;
        i = i + 1;
