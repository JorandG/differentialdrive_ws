...
function Reall = Reallocation(num_service_tasks, num_tasks, num_agents, num_filling_boxes, num_robots, service_time, timeReall, humanTime_filling, ReAll)
    ...
    Paux = optimvar('Paux', [(sum(1:num_tasks)-1)*num_robots],'Type', 'integer', 'LowerBound',0,'UpperBound',1 );
    timeS = optimvar('timeS', [1 num_tasks] ,'LowerBound', 0);
    timeF = optimvar('timeF', [1 num_tasks] ,'LowerBound', 0);
    invprod = optimvar('invprod', [num_tasks, num_robots] ,'LowerBound', 0);
    veloc = optimvar('veloc', [num_tasks, num_robots] ,'LowerBound', 0);
    ...
    if first_allocation == 2
        ...
        prob.Constraints.humanduration4 = timeFh(idx_to_consider_h) == timeSh(idx_to_consider_h) + humanTime_filling(idx_to_consider_h);

    elseif first_allocation == 0
        % Tasks duration
        invdist = invprod.*dist;
        for r=1:num_robots
            inversedistance = inversedistance + invdist(:,r);
        end
        inverse_dist = repmat(inversedistance, 1, num_robots);
        ...
        prob.Constraints.duration3 = invprod(idx_going_tasks,:) >= X(idx_going_tasks,:).*repmat((inv_vel_max)', num_tasks,1);
        prob.Constraints.duration4 = invprod(idx_going_tasks,:) <= X(idx_going_tasks,:).*repmat((inv_vel_min)', num_tasks,1);
    ...
    parallelism = optimconstr(2*(sum(1:num_tasks)-1)*num_robots);
    r=1;
    l = 1;
    for k=1:num_robots
        for i=1:num_tasks
...
function AllUpdated = updateSchedule(All, Alloc, humanTime_filling, dist, vel_min, vel_max, inv_vel_min, inv_vel_max, idx_depot_tasks, idx_going_tasks, idx_to_ignore_r, idx_to_ignore_h, agents_ordered_allocation, service_time)
    ...
    for i=1:num_agents*num_filling_boxes
        ...
        if (ismember(curr_hum,idx_going_tasks))
            dep = curr_hum+num_agents*num_filling_boxes-1;
            serv = curr_hum+num_agents*num_filling_boxes-2;
            wait = curr_hum+num_agents*num_filling_boxes-3;
        end
        %%%%%%%%%%%%%%% service_time
